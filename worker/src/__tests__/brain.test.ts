...
class FakeBrainDb {
  ...
  async rpc(name: string, args: Record<string, unknown>) {
    ...
    for (const write of writes) {
      const existing = this.rows.find((row) => row.account_id === accountId && row.path === write.path);
      ...
      const priorSaved = this.rows.find((row) => row.account_id === accountId && row.path === path);
      ...
      const index = this.rows.findIndex((item) => item.account_id === accountId && item.path === path);
...
describe("BrainStore", () => {
  it("round-trips a batch through exactly one atomic RPC and appends history", async () => {
    ...
    const result = await writeVariables(client, accountA, [
    ...
    expect(db.rpcCalls[0]).toMatchObject({ name: "write_brain_variables", args: { p_account_id: accountA } });
    expect(result.history).toHaveLength(2);
    expect(db.history).toHaveLength(2);
    expect(await readVariables(client, accountA)).toHaveLength(2);
  });

  it("preserves user truth and creates a machine contradiction with history", async () => {
    const db = new FakeBrainDb();
    const client = db.asSupabase();
    await writeVariables(client, accountA, [{ path: "positioning.one_liner", value: "User truth", confidence: "high" }], { source: "user_stated" });
    const result = await writeVariables(client, accountA, [{ path: "positioning.one_liner", value: "Scraped guess", confidence: "medium" }], { source: "scraped" });

    expect(result.contradictions).toHaveLength(1);
    const rows = await readVariables(client, accountA);
  ...
  it("allows machine-over-machine replacement and user writes always win", async () => {
    const db = new FakeBrainDb();
    const client = db.asSupabase();
    await writeVariables(client, accountA, [{ path: "canvas.key_resources", value: "old", confidence: "low" }], { source: "scraped" });
    await writeVariables(client, accountA, [{ path: "canvas.key_resources", value: "new", confidence: "high" }], { source: "mcp_pull:crm" });
    await writeVariables(client, accountA, [{ path: "canvas.key_resources", value: "override", confidence: "high" }], { source: "user_override" });
    await writeVariables(client, accountA, [{ path: "canvas.key_resources", value: "later machine", confidence: "high" }], { source: "workflow:positioning@v1#s1" });
  ...
  it("isolates reads and writes by account", async () => {
    const db = new FakeBrainDb();
    const client = db.asSupabase();
    await writeVariables(client, accountA, [{ path: "canvas.channels", value: ["A"], confidence: "high" }], { source: "user_stated" });
    await writeVariables(client, accountB, [{ path: "canvas.channels", value: ["B"], confidence: "high" }], { source: "user_stated" });

    expect((await readVariables(client, accountA)).map((row) => row.value)).toEqual([["A"]]);
    expect((await readVariables(client, accountB)).map((row) => row.value)).toEqual([["B"]]);
  ...
  it("supports prefix and exact-path reads without dropping account scope", async () => {
    const db = new FakeBrainDb();
    const client = db.asSupabase();
    await writeVariables(client, accountA, [
      { path: "canvas.channels", value: ["Direct"], confidence: "high" },
      { path: "positioning.one_liner", value: "Focused", confidence: "medium" },
    ], { source: "scraped" });

    expect((await readVariables(client, accountA, { prefix: "canvas." })).map((row) => row.path)).toEqual(["canvas.channels"]);
    expect((await readVariables(client, accountA, { paths: ["positioning.one_liner"] })).map((row) => row.path)).toEqual(["positioning.one_liner"]);
  });
});
